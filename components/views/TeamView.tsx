    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:14}}>
      {members.map(m=>{const mine=projects.filter(p=>p.assigneeId===m.id||p.managerId===m.id);return(
        <div key={m.id} style={{background:"#fff",border:"1px solid #E5E7EB",borderRadius:12,padding:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}><Avatar member={m} size={40}/><div><p style={{fontWeight:600,fontSize:14,color:"#111",margin:0}}>{m.name}</p><p style={{fontSize:12,color:"#9CA3AF",margin:"2px 0 0",textTransform:"capitalize"}}>{m.role} - {mine.length} videos</p></div></div>
