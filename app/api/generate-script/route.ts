...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.ANTHROPIC_API_KEY) return NextResponse.json({ error: "ANTHROPIC_API_KEY not configured in Vercel environment variables" }, { status: 500 });
...
Keep under 300 words. Tone: ${narratorStyle || "professional"}.`;
    ...
    const userContent = context
      ? `Video title: ${title}\n\nSource material:\n\n${context}`
      : `Video title: ${title}\n\nNo documents provided — write a structured template.`;

    const res = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": process.env.ANTHROPIC_API_KEY!, "anthropic-version": "2023-06-01" },
      body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1200, system, messages: [{ role: "user", content: userContent }] }),
    });
